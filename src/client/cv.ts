...
import { hexToRgb, rgbToHsv } from "../util";
...
const pBtn = document.getElementById("togglePause") as HTMLInputElement;
  ...
  camVideo.srcObject = await navigator.mediaDevices.getUserMedia({
    video: {
      deviceId: devices.filter(
        (dev) => dev.label.toLowerCase().indexOf("hd") !== -1,
      )[0].deviceId,
    },
  });
}
...
function setup() {
  pBtn.onchange = (e: any) => {
    ...
    if (camVideo.paused && e.target.checked) {
      camVideo.play();
    } else {
      camVideo.pause();
    }
  ...
  lowPicker.onchange = (e) => {
    ...
    lowerHsvYellow = [(h - 40 + 255) % 255, s * 0.45, v * 0.35, 0];
